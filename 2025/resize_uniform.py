...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=("Resize images in a folder so that, when sorted, their widths tend to follow a uniform "
                     "distribution between a specified minimum and maximum. The overall target mean is (min+max)/2. "
                     "No image is ever upscaled; an image is only reduced if its computed uniform target is lower "
                     "than its original width.")
    )
    parser.add_argument("--min_width", type=int, default=20, help="Minimum target width (default=20)")
    parser.add_argument("--max_width", type=int, default=200, help="Maximum target width (default=200)")
    parser.add_argument("--mu", type=float, default=None,
                        help="Desired mean width (default=(min_width+max_width)/2); note that for a perfect uniform distribution, mu must equal (min+max)/2")
...
def main():
    ...
    output_folder = "/media/slsecret/E624108524105B3F/Users/simon/Downloads/cars/prio_resized2/"
    min_width = args.min_width
    max_width = args.max_width

    # Set desired mean (mu) if not provided. For a uniform distribution between min and max, the mean must be (min+max)/2.
    desired_mu = (min_width + max_width) / 2 if args.mu is None else args.mu
