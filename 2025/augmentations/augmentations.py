...
def adjust_brightness(image):
    # Random brightness adjustment from -90 to +90
...
def adjust_contrast_blend(image):
    # Random contrast reduction by blending with gray
...
def apply_sun(image):
    # Simulates sun flare and glare with random ROI and intensity
    h, w = image.shape[:2]
    ...
    roi_x1 = random.uniform(0.1, 0.4)
    roi_y1 = random.uniform(0.1, 0.4)
    roi_x2 = random.uniform(0.6, 0.9)
    roi_y2 = random.uniform(0.3, 0.5)
    aug = A.Compose([
        RandomSunFlare(
            flare_roi=(roi_x1, roi_y1, roi_x2, roi_y2),
