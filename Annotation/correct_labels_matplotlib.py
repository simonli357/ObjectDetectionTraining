...
# name1 = "../test1"
IMAGE_FOLDER = "/home/slsecret/Downloads/bfmc_data/images/" + name1
LABEL_FOLDER = "/home/slsecret/Downloads/bfmc_data/images/" + name1 + "_labels"
TRASH_FOLDER = "/home/slsecret/Downloads/bfmc_data/images/" + name1 + "_trash"
...
CLASS_COLORS = ['g', 'b', 'r', 'c', 'm', 'y', 'gray', 'orange', 'olive', 'teal', 'maroon', 'navy', 'purple']
...
class BoundingBox:
    ...
    def draw(self, ax):
        ...
        rect = patches.Rectangle((self.x, self.y), self.w, self.h, linewidth=2, edgecolor=color, facecolor='none')
        ax.add_patch(rect)
        ax.text(self.x, self.y - 5, CLASS_NAMES[self.class_id], color=color, fontsize=8)
...
class AnnotationApp:
    ...
    def load_image(self):
        ...
        if os.path.exists(label_path):
            with open(label_path) as f:
                for line in f:
                    cid, x, y, w, h = self.yolo_to_box(line)
    ...
    def yolo_to_box(self, line):
        parts = line.strip().split()
        cid = int(parts[0])
        xc, yc, w, h = map(float, parts[1:])
        x = int((xc - w / 2) * self.img_w)
        y = int((yc - h / 2) * self.img_h)
        return cid, x, y, int(w * self.img_w), int(h * self.img_h)
